...
nlp = spacy.load("en_core_web_lg") # here we are loading the model once as it is an expensive operation hence why we load it at the module level. The model chosen is a large English language model trained on web text. It includes a pre-trained NER component that can already recognize locations, people, organizations and so o
